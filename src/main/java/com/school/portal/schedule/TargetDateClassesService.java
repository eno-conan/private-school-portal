// -----------------------------------------------------------------------------
// TargetDateClassesService.java
// -----------------------------------------------------------------------------
//
// 本ファイルについて
// * getTargetDateClassSchedule()：特定の日付をパラメータとして、
// その日の実施予定（実施済）の授業をstudent_schedule_normalテーブルから取得
//
// * prepareClassInfo()：getTargetDateClassScheduleメソッドで取得したデータについて、
// 必要なデータのみ抽出する処理を行う。
//
...
class TargetDateClassesService {
  ...
  List<Map<String, Object>> getTargetDateClassSchedule(final Date date) {
    // 本日の授業に関する情報取得
    List<StudentScheduleNormal> classList =
        studentScheduleNormalRepository.findAllByClassDateOrderByTimeTableNormalAsc(date);
